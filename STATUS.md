...
Track 1 (parameter recovery) at **v3.0 multi-AOI arc closed, 5/6 ceiling characterized as parameter conservation** — DarwinDiff is a gradient-based replacement for ECCO-Darwin's Green's-functions calibration at the same parameter scope (Carroll's 6). 5-PFT training runs ~80 sec on RTX 5090 Laptop; multi-AOI joint training adds ~30s per AOI. Cumulative arc:
...
- **v2.7 — 2-layer box model (PR #42 merged 2026-05-17 at `2afc1d8`):** extends Carroll-6 to a 2-layer (0–50 m + 50–1000 m) box with sinking-POC iron + Kz + subsurface remineralisation; adds a second GEOTRACES MSE loss on the 50–1000 m bin. Greptile caught an iron double-count in the initial integrator (`fe_uptake` + explicit sinking-POC term were both subtracting iron from DFe_1, then crediting it twice to L2). With the fix in place, the corrected v2.7 result is **NOT a leap over v2.6**: aggregate 3-4/6 Cal-grade, the same range. The pre-fix "3/3 Excellent alpfe at 0.011 off" headline is retracted as a bug artifact. The real contribution is a vetted 2-layer integrator + empirical confirmation that subsurface DFe anchoring alone does NOT unblock scav_rat (still drifted at ~0.87 off Carroll regardless of subsurface weight).
- **v2.8 — Darwin v5 pickup ICs + L2 POC observation loss (PR #45 merged):** swaps literature-default initial conditions for Darwin v5 pickup-derived per-cell ICs at the v2.7 anchor (5 inorganics × 2 layers), and adds a z-score POC_L2 loss term constraining the integrator's L2 POC trajectory. Real Darwin Eq Pac surface FeT is **30× lower** than the literature default; subsurface POC is **8× higher**. At anchor (Darwin IC, sub_w=1.0, POC_SUB_W=3.0, n=10): **scav_rat 7.66e-7, 0.403 off Carroll, 7/10 Cal-grade, 4/10 Excellent** — project-first reproducible scav_rat recovery. **Aggregate 6/10 seeds at 4/6 Cal-grade** — double the v2.7 best. Cost: bimodal degeneracy in the alpfe ↔ scav_rat axis — POC_SUB_W is the lever. R_PICPOC still 0/10 Cal-grade. Full record at [`docs/findings/v2.8_darwin_ic_poc_sub.md`](docs/findings/v2.8_darwin_ic_poc_sub.md).
- **v3.0 multi-AOI arc (PRs #45 - #57 merged, PRs #58 + #59 in review/draft, 2026-05-17 → 2026-05-18):** joint training across Eq Pac + N Atl Subpolar with a shared Carroll-6. Implementation arc covers AOI parameterization, AOI ID env channel (PR #53 — first single 5/6 seed), GEOTRACES POC absolute loss (PR #52), DINN capacity sweep (PR #54 — first joint diatomgraz Cal-grade at hd=32, NAtl_W=2.0), dual cellweighted/aoiweighted recovery reporting (PR #56), and an overnight CHL1_W_EXTRA × AOI weight sweep (PR #57, 80 JSONs across 8 configs). **PR #57 best config baseline (n=15): 7/15 at 5/6 Cal-grade (mean_cal = 3.93)** — established the v3.0 plateau. PR #58 tested the per-AOI-DINNs + consistency penalty architectural hypothesis (`PER_AOI_DINN=1`, `CONSISTENCY_LAMBDA ∈ {0.01, 0.1, 1.0, 10.0}` × n=10): **0/40 at 6/6, falsified**. PR #59 tested PIC_ABS_W + POC_ABS_W absolute-magnitude anchors across three sweeps (PIC alone; paired heavy; paired light): **all configurations underperform baseline on aggregate mean_cal**; R_PICPOC can be magnitude-pinned per-AOI but at the cost of iron-pair Cal collapse. **Structural finding (nb32):** across 17 distinct configurations, no anchored config beats baseline. The observations support ~5 effective constraints on 6 parameters — the 6th is always the "residual sink", and loss weighting decides which: baseline → `diatomgraz`; per-AOI DINN → `R_PICPOC`; paired anchors → `alpfe + scav_rat`. **The actual baseline binding parameter is `diatomgraz`** (2/15 Cal in baseline; 6/7 5/6-miss seeds drop diatomgraz). Earlier mid-session framing of "R_PICPOC stuck at 0.014" was a per-AOI-DINN 5/6-miss-seed artifact, not baseline state — corrected in PR #59 description, `notebooks/32_v3_0_param_learner_ceiling.ipynb`, and the v3.0 thread.
...
**Fine sweep at finer GEOTRACES_W resolution (0.15 / 0.20 / 0.25) confirms no sweet spot.** scav_rat sits at 0.89–0.92 off Carroll across all weights from 0.15 to 1.0; the alpfe-scav_rat tradeoff at v2.6 surface-only is genuinely structural, not a weight-tuning artifact. w=0.25 produces equivalent results to w=0.30 (Smallgrow Cal-grade, alpfe Cal-grade, 4/6). The earlier ceiling claim of "4/6 with Smallgrow Cal-grade — period" stood for v2.6 and v2.7 (surface obs only, literature ICs). **v2.8 broke that ceiling on 2026-05-17** by adding Darwin pickup ICs + an L2 POC observation loss term; the new local-GPU achievable scoreboard is 6/10 seeds at 4/6 Cal-grade with scav_rat reproducibly recovered.

**Track 1 progress on a single GPU**: v2.8 demonstrates project-first scav_rat recovery without cluster compute. The next single-GPU lift is v3.0 multi-AOI training (PR #44 design merged 2026-05-17) to test whether cross-regime coverage breaks the v2.8 bimodal degeneracy. B200 cluster burn-in pitch sent to MIT ORCD 2026-05-10 — cross-basin (non-HNLC, e.g., N Atlantic) remains the cluster-gated path to full identifiability. Cluster prep documented in [`docs/cluster_setup.md`](docs/cluster_setup.md).
...
- [x] **PR #42 — v2.7 2-layer Carroll-6 box (corrected after Greptile P1)** merged at `2afc1d8` (2026-05-17). Aggregate 3-4/6 Cal-grade; subsurface DFe anchoring alone does NOT unblock scav_rat. Vetted 2-layer integrator on main.
- [x] **PR #43 — STATUS.md honest n=10 + PR-state refresh** merged at `503e05b` (2026-05-17). Retracts the n=5 "5/5 Excellent" Smallgrow claim; codifies honest n=10 framing.
- [x] **PR #44 — v3.0 multi-AOI scoping doc** merged at `aea8f9f` (2026-05-17). Planning only, no code.
- [x] **PR #45 — v2.8 Darwin v5 ICs + L2 POC observation loss** merged. Project-first reproducible scav_rat recovery.
- [x] **PRs #46-#57 — v3.0 multi-AOI implementation arc** merged 2026-05-17 / 2026-05-18. Includes #47 (N Atl single-AOI test, first R_PICPOC Cal), #48 (joint multi-AOI baseline), #52 (GEOTRACES POC absolute loss), #53 (AOI ID channel, **first 5/6**), #54 (DINN capacity sweep, first joint diatomgraz Cal), #56 (dual cellweighted/aoiweighted recovery reporting), #57 (overnight CHL1_W_EXTRA × AOI weight sweep, 80 JSONs across 8 configs, no 6/6).
- [x] **PR #58 — v3.0 per-AOI DINNs + CONSISTENCY_LAMBDA penalty** ready for review. λ sweep × n=10: 0/40 at 6/6; best λ=0.1 only 3/10 at 5/6. **Architectural-ceiling hypothesis falsified.**
- [x] **PR #59 — v3.0 PIC_ABS + POC_ABS paired anchors + `notebooks/32_v3_0_param_learner_ceiling.ipynb` arc analysis** draft. Three sweeps (PIC alone; paired heavy; paired light) all underperform baseline. Recommended NOT for merge as v3.0 default; tool + diagnosis. nb32 is the v3.0 close-out deliverable with three plots + summary tables + corrected diagnosis (diatomgraz is the baseline binding param).
- [ ] **POSi (biogenic silica) loss + box-model state extension** — empirically-justified next step. Diatom-specific tracer (~1280 finite GEOTRACES bSi values); directly attacks the baseline binding parameter (`diatomgraz`); does not compete with iron-pair budget. ~1-2 hr code lift.
- [ ] **3rd AOI: Southern Ocean Pacific sector** — adds another regime contrast; per-AOI DINN tool from PR #58 makes 3-AOI nearly free.
- [ ] **Ocean color loader (Smallgrow target)** — exact URL pending from Jon.
- [ ] **GLODAPv2.2023 mapped upgrade** — refinement vs v2.2016b currently used.

...
**Last updated:** 2026-05-17 (Track 1 **v2.8 — Darwin v5 ICs + L2 POC observation loss**: project-first reproducible `scav_rat` recovery. At anchor (`DARWIN_IC=1, GEOTRACES_W=0.3, GEOTRACES_SUB_W=1.0, POC_SUB_W=3.0, PINN_drift_w=3.0, n=10 seeds`): `scav_rat` **7.66e-7 ± 4.23e-7 (0.403 off Carroll, 7/10 Cal-grade, 4/10 Excellent)** — vs prior project-wide 0.91 off in every config tested. Aggregate **6/10 seeds at 4/6 Cal-grade** — twice the previous best (3/10). First-ever Biggrow Excellent (seed 6). Cost: alpfe trades from project-best at sub_w=1.0 (7/10 Excellent under Darwin IC alone) into Drifted (2/10 Cal-grade) — empirical evidence of a **bimodal degeneracy** in the (alpfe, scav_rat) loss surface; `POC_SUB_W` selects the basin. R_PICPOC still 0/10 Cal-grade — PIC obs remains the next unblock. Full record at [`docs/findings/v2.8_darwin_ic_poc_sub.md`](docs/findings/v2.8_darwin_ic_poc_sub.md). PRs merged 2026-05-17: #42 v2.7 2-layer box (2afc1d8), #43 STATUS honest-n=10 (503e05b), #44 v3.0 multi-AOI scoping (aea8f9f). v2.8 PR in flight on branch `v2.8-darwin-ic-poc-sub`.)

## Where we are in one line

Track 1 (parameter recovery) at **v2.8 — Darwin ICs + L2 POC obs era** — DarwinDiff is a gradient-based replacement for ECCO-Darwin's Green's-functions calibration at the same parameter scope (Carroll's 6). 5-PFT training runs ~80 sec on RTX 5090 Laptop. Cumulative arc:
...
- **v2.8 — Darwin v5 pickup ICs + L2 POC observation loss (branch `v2.8-darwin-ic-poc-sub`, PR in flight):** swaps literature-default initial conditions for Darwin v5 pickup-derived per-cell ICs at the v2.7 anchor (5 inorganics × 2 layers), and adds a z-score POC_L2 loss term constraining the integrator's L2 POC trajectory. Real Darwin Eq Pac surface FeT is **30× lower** than the literature default; subsurface POC is **8× higher**. Both mismatches were the proximate cause of the alpfe / scav_rat undershoots in v2.6/v2.7. At anchor (Darwin IC, sub_w=1.0, POC_SUB_W=3.0, n=10): **scav_rat 7.66e-7, 0.403 off Carroll, 7/10 Cal-grade, 4/10 Excellent** — project-first reproducible scav_rat recovery, after every prior config hit ~0.91 off Carroll. **Aggregate 6/10 seeds at 4/6 Cal-grade** — double the v2.7 best. First-ever Biggrow Excellent (seed 6). Cost: bimodal degeneracy in the alpfe ↔ scav_rat axis — POC_SUB_W is the lever that switches the optimizer between an "alpfe-correct, scav_rat low" basin and a "scav_rat-correct, alpfe low" basin. Carroll's joint (alpfe=0.928, scav_rat=6e-7) sits at neither minimum, identifiability-blocked by the 6-param/11-target structure. R_PICPOC remains 0/10 Cal-grade across every POC_SUB_W tested — PIC observation is the next unblock. Full record at [`docs/findings/v2.8_darwin_ic_poc_sub.md`](docs/findings/v2.8_darwin_ic_poc_sub.md).
...
- [x] **PR #44 — v3.0 multi-AOI scoping doc** merged at `aea8f9f` (2026-05-17). Planning only, no code. Implementation gated on v2.7 (now satisfied); v2.8 is opt-in.
- [ ] **v2.8 — Darwin v5 ICs + L2 POC observation loss** (branch `v2.8-darwin-ic-poc-sub`, PR in flight). Project-first scav_rat recovery (4/10 Excellent at 0.3-4.5% off Carroll); 6/10 aggregate 4/6 Cal-grade; bimodal degeneracy in alpfe↔scav_rat revealed; POC_SUB_W is the basin selector. R_PICPOC still 0/10 — PIC obs next.

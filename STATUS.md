...
**Last updated:** 2026-05-10 (Track 1 v1.8 — nb18 cross-basin DINNDeep + nb19 multi-tracer joint loss).

## Where we are in one line

Track 1 (parameter recovery) at **v1.8** — DarwinDiff functions as a *differentiable-physics diagnostic* for ECCO-Darwin BGC calibration. Recovered Carroll-6 means are order-of-magnitude correct (within 0.5–3× of Carroll's published values), not bit-for-bit calibration-grade. Coverage: 3 basins × 4 targets (Chl, NO₃, FeT, plus joint-loss POC + PIC). Per-cell DINN baseline (1×1 conv, ~400 params, SST input) fits the spatial pattern where global-scalar produces a constant prediction — strongest single-fit r = 0.979 on N Pacific NO₃. DINNDeep saturates fit quality (r ≈ 1.000) on every (AOI × target) combo tested — Eq Pac FeT (nb15), N Pac Chl (nb18), and 4-tracer joint loss (nb19) — but with degenerate parameter recovery. Cross-validation: random hold-out r = 0.995 (passes), block hold-out r = 0.301 (fails extrapolation, nb16; reproducible across 5 seeds, nb17). Multi-tracer joint loss (nb19) partially collapses the degeneracy — 3/6 Carroll-6 parameters closer to Carroll's published values; iron pair (alpfe, scav_rat) remains 2–3× off. Track 2 (neural emulator) not started. **Track 1 in a stable position on local hardware**; box-model carbonate extension (nb20 candidate) is the largest remaining local experiment; full-ocean / time-resolved scope is gated on cluster compute.
...
- **Track 1** (parameter recovery via differentiable physics) — at **v1.8**. Per-cell DINN baseline fits the spatial pattern where global-scalar produces a constant prediction; DINNDeep saturates fit quality on every (AOI × target) combo tested with degenerate parameter recovery (nb15, nb18); cross-validation confirms DINNDeep is interpolation-only across spatial blocks (nb16); ensemble disagreement detects extreme errors but not extrapolation territory (nb17); multi-tracer joint loss partially collapses parameter degeneracy with 3/6 Carroll-6 means closer to Carroll's published values (nb19). Iron pair (alpfe, scav_rat) remains underconstrained without depth-resolved observations across all configurations tested. **In a stable position on local hardware**; box-model carbonate extension (nb20 candidate) is the largest remaining local experiment that could meaningfully shift the assessment. Full-ocean / time-resolved scope is gated on cluster compute — cluster transfer prep documented in [`docs/cluster_setup.md`](docs/cluster_setup.md).

...
Data: BCO-DMO 922468 (CC-BY-4.0), cached at ``data/xu_weber_2021/`` (gitignored). Fields as flat CSV:
- ``Aldep.txt``  : soluble Al deposition, mmol Al/m^2/yr, serialized [91 lat x (180 lon * 60 configs)].
...
should be refined regionally from GEOTRACES GA03 aerosol solubility (Phase-1). It sets the absolute
alpfe value; the anchor's identifiability leverage comes from the PATTERN, which is ratio-independent.
...
def load_aldep_field(xw_dir: str | Path = DEFAULT_XW_DIR):
    """Load the soluble-Al deposition ensemble. Returns (mean, std, lat, lon) on the native 2-deg grid.
...
def soluble_fe_deposition_aoi(
    ...
    *,
    ...
    Bilinearly regrids the 2-deg Al-inverse mean/std onto the AOI grid (integer-degree centres, matching
    ``darwindiff.llc270_loader.bin_to_1deg_grid`` / the box caches), then converts Al -> soluble Fe by
    ``fe_al_molar``. Returns (dfe_dep [ny,nx], dfe_spread [ny,nx], lats, lons) in mmol Fe/m^2/yr.

    The ensemble spread is the per-cell 1-sigma the source-anchor prior should use (down-weight cells
    where between-config spread is large, per the red-team). ``fe_al_molar`` scales both mean and spread.
    """
    from scipy.interpolate import RegularGridInterpolator

    mean, std, lat, lon = load_aldep_field(xw_dir)
    # RegularGridInterpolator needs finite values; fill NaN with 0 (no deposition) for interpolation,
    ...
    dep = f_mean(pts).reshape(LAT.shape)
    spread = f_std(pts).reshape(LAT.shape)
    covg = f_cov(pts).reshape(LAT.shape)

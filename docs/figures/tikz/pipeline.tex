...
\begin{tikzpicture}[node distance=6mm and 9mm]
  ...
  \node[ddlabel, text=ddIdent, anchor=west] (t1) at (0,2.25) {DarwinDiff --- differentiable (PyTorch autograd)};
  ...
  \draw[flow] (par) -- (box);
  \draw[flow] (box) -- (loss);
  \draw[grad] (loss.north) .. controls ([yshift=13mm]loss.north) and ([yshift=13mm]dinn.north) ..
        node[midway, above=1.5pt, ddtiny, text=ddIdent] {one backward pass $\rightarrow$ all six $\partial\mathcal{L}/\partial\theta$}
        (dinn.north);

% fig4_architecture_bw.tex
%
% MONOCHROME variant of fig4_architecture_dimensional.tex. Identical geometry; the four
% \colorlet lines below are the only difference. Regenerate by copying the dimensional
% source and replacing those four lines, or the two will drift apart.
%
...
%
% ROLE IS CARRIED BY OUTLINE STRUCTURE, not by grey level.
%
% An earlier version of this file separated "measured" from "the model" by stroke darkness
% alone, black!61 against black!74. Thirteen points of grey is not a distinction a reader
% resolves -- certainly not after a photocopier -- and it is the v1 palette's mistake in
% miniature: a difference that exists in the file and not in the eye. So the two categories
% now carry the SAME stroke value and are told apart structurally:
%
%   measured, not ours   DOUBLE outline, rounded             (forcing in, observations out)
%   the model            single outline, rounded             (network, parameter field, box)
%   a CHOICE we made     single HEAVY outline, SHARP corners (the collapse)
%   the gradient         dashed, and nothing else is dashed
%
% Each of those survives any reproduction that preserves black and white at all, which is
% more than can be said for a luminance ladder. Fills stay faint tints so they read as
% grouping, never as category.
%
% This is the one place the project's own greyscale rule is satisfied by construction rather
% than by audit -- darwindiff-tikz-v2.sty documents that the v1 palette FAILED it, sky-blue
% against orange at a greyscale contrast ratio of 1.02.
%
...
% ---------------------------------------------------------------------------------------
%
%   pdflatex fig4_architecture_bw.tex
%
\documentclass[tikz,border=10pt]{standalone}
\usetikzlibrary{arrows.meta,positioning,calc,fit,backgrounds}

\colorlet{myblue}{black!55}
\colorlet{mygreen}{black!55}  % deliberately EQUAL to myblue -- the double outline distinguishes
\colorlet{myorange}{black!80}
...
% #1 (optional) extra options for the SHEET OUTLINE only -- this is where `double` goes, so a
% measured slab and a model slab differ by edge structure rather than by grey.
\newcommand{\slabc}[4][]{%
  \begin{scope}[shift={(#2,#3)}, x={(0.215cm,-0.095cm)}, y={(0.215cm,0.128cm)}]
    \fill[black, opacity=0.12, transform canvas={xshift=1.5pt, yshift=-1.5pt}]
      (-2,-2) -- (2,-2) -- (2,2) -- (-2,2) -- cycle;
    \fill[#4!16, draw=#4!55!black, line width=0.45pt, #1]
      (-2,-2) -- (2,-2) -- (2,2) -- (-2,2) -- cycle;
    \foreach \i in {-1,0,1}{
      \draw[#4!38!black, line width=0.2pt] (\i,-2) -- (\i,2);
      \draw[#4!38!black, line width=0.2pt] (-2,\i) -- (2,\i);
    }
  \end{scope}%
}
...
\begin{tikzpicture}[
  ...
  choicebox/.style={draw=myorange!65!black, line width=1pt, fill=myorange!12,
                  ...
                  font=\sffamily\fontsize{6.5}{8}\selectfont, text=myink},
  realbox/.style ={draw=mygreen!50!black, line width=0.6pt, rounded corners=1.5pt,
...
\slabc[double, double distance=0.9pt]{0}{0}{mygreen}
...
% rides the descent with a white fill that breaks the dashes. Sitting it at (13.0,-1.05)
...
% ============================================================ legend
% Outline structure is doing the semantic work here, so it needs declaring. Four roles, in
% the order they appear along the flow. Each swatch uses the SAME grammar as the stage it
% names -- double edge for measured, single for model, heavy sharp for the choice, dashed for
% the gradient -- so the legend is checkable against the figure rather than merely adjacent.
% The swatches are 0.34 x 0.22, not the 0.22 x 0.15 of the colour version. A double edge needs
% room to read as two lines; at the smaller size it degrades to a thick single one, which is
% precisely the ambiguity this variant exists to remove. Corner treatment is mirrored too --
% rounded for the two rounded stages, sharp for the sharp one.
\begin{scope}[shift={(0,-3.30)}]
  \fill[mygreen!16, draw=mygreen!55!black, line width=0.45pt, rounded corners=1.2pt,
        double, double distance=1.1pt] (0,-0.11) rectangle ++(0.34,0.22);
  \node[note, anchor=west] at (0.48,0) {measured, not ours};

  \fill[myblue!16, draw=myblue!55!black, line width=0.45pt, rounded corners=1.2pt]
        (3.05,-0.11) rectangle ++(0.34,0.22);
  \node[note, anchor=west] at (3.53,0) {the model};

  \fill[myorange!12, draw=myorange!65!black, line width=1pt] (5.35,-0.11) rectangle ++(0.34,0.22);
  \node[note, anchor=west] at (5.83,0) {a choice we made};

...
%      loss; an iron-budget chip also reads xT and has its own arrow into L.
...
%     sansmathfonts), and Euler is the one unserifed Greek it has. The slash in dL/dw is the text
%     slash, and the elision is three drawn dots, so no Computer Modern glyph is left anywhere;
...
%   - extra red heads sit under the two step boxes (pos 0.402 / 0.644), each step hanging a
%     hairline onto the backward pass, echoing the junction hairlines in zone A; 'through every
%     step' is centred under the span of those two heads; every hairline is ddRule 0.5 pt;
...
%     iron-budget chip sits at y 18-24 and the x_T -> chip elbow leaves from x 126.4, not 126;
...
\newcommand{\mL}{\obl{\usefont{U}{eus}{m}{n}L}}      % Euler Script L, medium (in dL/dw)
...
\mshadow{[rounded corners=1.2pt] (11,10) rectangle (48,16)} % shared weights
...
\mshadow{(131.5,18) rectangle (148.5,24)}      % iron-budget chip
\mshadow{(162,13) circle (4.5)}
...
\draw[fixedbox] (131.5,18) rectangle (148.5,24);
...
% x_T -> iron budget
\draw[flow] (126.4,29) |- (131.5,21);  % 0.4 right of spec: clears the x_T label
% the collector: all three cards feed L. Down the back-right corners, then into the top of L
\draw[flow, rounded corners=1.6mm] (154,\dO+4) -- (154,23) -- (162,23) -- (162,17.6);
...
\draw[flow] (148.5,21) -- (158.04,15.35);
% each step hangs a hairline onto the backward pass: the gradient goes back through every step
\draw[ddRule, line width=0.5pt] (87,32.5) -- (87,13.8);
\draw[ddRule, line width=0.5pt] (113.5,32.5) -- (113.5,13.8);
...
\node[font=\fLabel] at (77,25.7) {$x_0$};
...
\node[font=\fSmall] at (140,21) {iron budget};
\node[font=\fontsize{12}{14}\selectfont] at (162,13) {\mLb};  % 12 pt: the loss symbol fills its node

\node[font=\fSmall, text=ddMute] at (101.45,9.3) {through every step};
\node[font=\fLabel] at (55.5,9.3) {\mpd\mL\textup{/}\mpd$w$};
\end{scope}
\end{tikzpicture}
\end{document}

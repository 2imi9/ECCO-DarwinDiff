...
**Status:** Planning doc only. **No code in this PR.** Implementation deferred until both PR #42 (v2.7 2-layer box) and the v2.8 depth-resolved POC work merge, so v3.0 builds on the stable post-fix integrator and the v2.8 POC constraint if v2.8 ships first.
...
| **Southern Ocean Pacific sector** *(new)* | −65° to −50° S, 180° to −100° W | ~1,500 | High-particle-flux, carbonate-rich, seasonal bloom | Biggrow, R_PICPOC, diatomgraz cross-check |
...
Combined loss is the **AOI-weighted sum**, where the weight is inversely proportional to ocean-cell count per AOI (so each AOI contributes equally to the gradient regardless of size). Default: equal weight 1.0 per AOI; expose `AOI_W_<aoi>` env vars for sweep experiments.

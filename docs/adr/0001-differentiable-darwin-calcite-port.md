...
> **STATUS: REJECTED / SUPERSEDED (2026-06-27).** This ADR's central thesis — that the differentiable Darwin calcite port is *"the only path to R_PICPOC / 6-of-6"* and that forward-model fidelity is *"THE WALL"* — is **REFUTED**. R_PICPOC is recoverable at 1° box scale with a *real* calcite anchor (Daniels CP:PP / MODIS PIC) plus the `RATIO_MAX=2` fix; the calcite port **and** native resolution were tested and did **not** help. The honest target is **4 observable params** (the growth pair is unobservable by construction); there is no 6/6 condition. The surrogate gap is *dimensional* (the 0-D box homogenizes spatial structure, CV→1e-15), not a calcite-physics wall. Retained as a decision record of the design considered and why it was not pursued. See `../../STATUS.md` and `../research_notes/2026-06-27_box_homogenization_DEFINITIVE.md`.

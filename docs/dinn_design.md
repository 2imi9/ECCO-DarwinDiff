...
- **The box model is a 5-tracer proxy of full Darwin 3.** `carroll6_step` integrates DFe + Ps + Pl + POC + PIC. Darwin 3 has 5 phytoplankton functional types (collapsed to Ps + Pl in the proxy) + 2 zooplankton + DOM + carbonate chemistry + more. The 5-PFT box (`carroll6_5pft.py`) and 2-layer extension (`carroll6_5pft_2layer.py`) close part of this gap. The binding limitation is **dimensional**: the 0-D box homogenizes spatial structure (tracer CV → ~1e-15 at convergence), so identifiability must come from real *absolute* anchors rather than box-vs-Darwin pattern fidelity. Of the 4 observable params, `alpfe` and `R_PICPOC` recover globally and `scav_rat` regionally (`geo1` holds the trio {`alpfe`, `scav_rat`, `R_PICPOC`} jointly in **25/50** seeds at n=50 under the honest per-AOI ≥2-of-3 metric, arithmetic collapse — 12/50 geometric — with `alpfe` 49/50, `R_PICPOC` 50/50 and the binding leg `scav_rat` 25/50; the 41/50 at 4000 epochs is arithmetic-only and un-auditable, and the 2026-08-04 2×2 showed that lever costs Southern Ocean accuracy; the n=10 `7/10` was the precursor); `diatomgraz` is regionally identifiable in the equatorial Pacific (40/100 at ≤10 % vs untrained 0/50) and recoverable once **MLD** is a DINN input channel (SST-only sits at chance; the 10/10 with MLD at n=10 is a 0.40-aggregate-band count the prior already passes, so contaminated-band evidence rather than recovery; the 35/50 count with the biogenic-silica diagnostic off is retired against a 34/50 untrained control), but only against Darwin's own chlorophyll — model-internal consistency, not independent real-data validation; the growth pair is excluded by construction (`Biggrow` unobservable; `Smallgrow` non-identifiable from time-mean observables only).

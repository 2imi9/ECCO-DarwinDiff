...
% --- abstract ---------------------------------------------------------------
\begin{abstract}
\noindent ECCO-Darwin's six-parameter biogeochemistry calibration uses Green's functions: expensive perturb-and-fit Jacobians that do not transfer when new observations come online. We adapt the Biogeochemistry-Informed Neural Network (BINN) pattern from terrestrial soil carbon to marine biogeochemistry. A small per-cell neural network maps environmental covariates to the six Carroll-N parameters, sigmoid-bounded into Carroll's published ranges and fed through a differentiable PyTorch reimplementation of Darwin's source-minus-sink (SMS) kernel; the observation loss backpropagates end-to-end. We report the v3.1 sweep: \textbf{856 seeds in 86 single-lever configurations}. Three findings, framed honestly. \textbf{First, partial recovery is real and reproducible:} the iron pair (\texttt{alpfe}, \texttt{scav\_rat}) recovers at \textbf{38/40 (95\%) Cal-grade} with our best configuration --- the first reproducible differentiable-parameter-recovery result for ECCO-Darwin biogeochemistry. \textbf{Second, a clear recoverability gradient across the Carroll-6:} \texttt{alpfe} (84\% of seeds Cal-grade) and \texttt{scav\_rat} (70\%) reliably recover; \texttt{Biggrow} (47\%) and \texttt{Smallgrow} (28\%) partially; \texttt{diatomgraz} (10\%) and \texttt{R\_PICPOC} (3\%) almost never. The \textbf{typical seed recovers 2 or 3 of 6 parameters} (80\% of seeds; modal bucket is 2/6 at 43\%, second bucket is 3/6 at 37\%; mean~=~2.41); 92\% of seeds reach 3/6 or fewer. \textbf{Third, a structural 5/6 ceiling:} 0/856 seeds reach all six jointly; only two unreproduced single-seed 5/6 events; a composition test of the two complementary 5/6 lever families regresses below either parent. The framework is therefore a \emph{partial} replacement of Green's-functions calibration --- not a drop-in for all six parameters yet, but a reproducible recovery of the iron pair with a characterized gradient and a predictive diagnostic for which new observation channels (satellite PIC for the carbonate counter-pump, additional GEOTRACES traverses) would unblock the remaining four.
\end{abstract}
...
Interpretation. The iron parameters are well-constrained by GEOTRACES IDP2025 surface and subsurface dissolved-iron observations. The phytoplankton growth rates are partially constrained by chlorophyll across the five functional types, but the 5-PFT $\to$ 2-PFT collapse in our box model dilutes the signal. The diatom palatability is essentially unconstrained at the box-model level. The PIC/POC ratio is essentially unrecoverable from v05 observations alone --- the carbonate counter-pump needs a dedicated PIC observation channel (e.g., satellite particulate inorganic carbon) that v05 does not include. This is consistent with the binary observation-anchor mutex in §\ref{sec:results-mutex}.
...
The result is \textbf{partial recovery with a characterized gradient}, not a drop-in replacement for Green's-functions calibration:

\begin{itemize}[noitemsep,topsep=2pt]
  \item \textbf{Reliable (2 of 6).} The iron pair (\texttt{alpfe} at 84\%, \texttt{scav\_rat} at 70\% of seeds Cal-grade) is reproducibly recoverable; the F2 configuration delivers 38/40 at $n{=}40$.
  \item \textbf{Partial (1 of 6 on average, occasionally 2).} The phytoplankton growth rates (\texttt{Biggrow} 47\%, \texttt{Smallgrow} 28\%) recover under favourable configurations but not consistently. This is where 2/6 seeds turn into 3/6 seeds.
  \item \textbf{Essentially unrecovered (2 of 6).} \texttt{diatomgraz} (10\%) is rarely recovered; \texttt{R\_PICPOC} (3\%) almost never. The carbonate counter-pump requires an observation channel (satellite PIC) that v05 does not provide, and the binary observation-anchor mutex (§\ref{sec:results-mutex}) shows why simply adding a PIC anchor in the wrong way actively breaks iron-pair recovery rather than helping.
\end{itemize}

The typical seed therefore lands at \textbf{2 or 3 of 6 Cal-grade} (80\% of seeds in those buckets combined; mean 2.41). The \textbf{5/6 ceiling} is a structural upper bound, not the typical envelope. It is supported by four independent pieces of evidence:
...
Two things, both honest about the partial-recovery state.
...
\textbf{(ii) A predictive diagnostic for the remaining four.} The recoverability gradient and the binary observation-anchor mutex jointly tell future obs designers exactly what is missing: the carbonate counter-pump needs a dedicated PIC observation channel that v05 does not include; the diatom palatability needs constraints the 2-PFT box-model proxy currently dilutes; the phytoplankton growth rates need richer PFT-specific Chl coverage. These predictions are testable a priori inside DarwinDiff's framework: drop in a candidate observation as a loss term, run a small sweep, see whether the gradient shifts.

\textbf{What this is \emph{not} yet:} a full replacement of Carroll's six-parameter calibration. We recover two; the other four require the framework extensions in the Next-steps section.

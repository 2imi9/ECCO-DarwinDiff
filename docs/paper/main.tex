...
Five chained waves of single-lever sweeps plus a Wave 6 composition test. \textbf{856 seeds across 86 configurations} after aggregation. Table~\ref{tab:summary} reports the headline counts; the central observation is that the \textbf{typical seed recovers 2 or 3 of the 6 Carroll-6 parameters} (80\% of seeds combined; modal bucket 2/6 at 43\%, runner-up 3/6 at 37\%), with 92\% at 3/6 or fewer.

\begin{table}[ht]
  \centering
  \caption{v3.1 sweep summary. ``Cal-grade'' means a recovered Carroll-6 parameter falls within $\pm 40\%$ of Carroll 2020's published optimum.}
  \label{tab:summary}
  \begin{tabular}{lr}
    \toprule
    Metric & Value \\
...
The Carroll-6 split into three recoverability tiers (Table~\ref{tab:gradient}). The iron pair (\texttt{alpfe}, \texttt{scav\_rat}) is reliably recoverable; the phytoplankton growth rates partially; the diatom palatability and PIC/POC ratio are almost never recovered. This gradient is the central scientific output of the sweep: it tells future observation-design which Carroll-6 parameters are constrained by the current v05 observation set and which are not.
...
The headline empirical finding: \textbf{0/856 seeds reach all six Carroll-6 parameters jointly at Cal-grade}. Only two seeds reach 5/6, both unreproduced. Figure~\ref{fig:ceiling} shows the Cal-grade count distribution; Figure~\ref{fig:per-param-dist} shows per-parameter recovery spread.
...
\textbf{Under the AOI ablation (§\ref{sec:results-aoi-ablation})}, the ``Hard'' tier dissolves: the 2-AOI \texttt{eqp}+\texttt{natl} configuration recovers \texttt{diatomgraz} at 85\% and \texttt{R\_PICPOC} at 20\% (median recovered value 0.044 vs.\ Carroll's 0.042), at the cost of dropping \texttt{scav\_rat} to 1\%. Four of the six parameters are therefore recoverable from existing v05 observations under at least one AOI configuration we tested; only \texttt{Smallgrow} stays below 25\% Cal-grade in every configuration. The ``Hard'' tier in the v3.1 sweep reflects the 3-AOI joint-loss landscape rather than an intrinsic obs-channel limit.

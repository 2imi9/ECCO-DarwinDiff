...
- **The surrogate gap is dimensional.** At uniform parameters the 0-D box relaxes to a spatially near-uniform state (tracer CV → ~1e-15, vs Darwin's O(1)): it cannot reproduce Darwin's circulation-driven spatial structure. So box-vs-Darwin *pattern* correlations are not fidelity metrics, and identifiability comes from real *absolute* anchors — which is precisely what makes the per-cell DINN load-bearing (a global scalar would give a flat box). `docs/research_notes/2026-06-27_box_homogenization_DEFINITIVE.md`.

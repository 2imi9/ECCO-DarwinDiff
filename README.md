...
- **Real-data validated** (2026-06, H200): the iron pair is independently preferred by **real GEOTRACES dissolved iron** (FIM/profile spine: `alpfe` 0.103 under the full loss vs **0.9997 ≈ Carroll** under real iron), and **`R_PICPOC` recovers ≥2-AOI in 50/50 seeds, Wilson 95% CI [0.93, 1.00]** against the Darwin-*independent* Daniels 2018 CP:PP anchor (not Darwin's own PIC) — the first real-data-anchored `R_PICPOC` recovery, breaking the circularity, `verify_run.py` exit 0.

...
def measure_peak(
    ...
    *,
    ...
    OOM is caught and recorded as ``oom=True`` with ``None`` bytes. With
    ``use_compile`` the per-step is ``torch.compile``'d (the production batched path,
    #119) so the measured constant reflects the *compiled* footprint, not the eager
    upper bound -- Linux/WSL only.
    """
    device = torch.device(device)
    # Compiling the per-step (not the rollout) mirrors how production integrates;
    # the integrator threads it through every forward-Euler step. Built per call so
    # each problem shape gets its own specialised graph.
    ...
    try:
        ...
        with torch.no_grad():
            ...
            _ = carroll6_5pft_2layer_integrate(state0, params, DT_DAYS, n_steps,
...
def fit_cellstep_scaling(records: list[MemRecord]) -> dict[str, float]:
    ...
    denom = n * sxx - sx * sx
    if denom == 0:  # all (cells*steps) products identical -> slope undefined
        return {"a_bytes": float("nan"), "b_bytes_per_cellstep": float("nan"),
                "r2": float("nan"), "n": float(len(xs))}
    b = (n * sxy - sx * sy) / denom
    a = (sy - b * sx) / n
    mean_y = sy / n
...
def _render_markdown(
    ...
    lines.append(
        ...
        f"`DINN → integrate → loss → backward` pass ({mode}, **pre-checkpointing**) "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--compile", action="store_true",
                        help="torch.compile the per-step -> measure the COMPILED peak "
    ...
    if args.md:
        ...
        args.md.write_text(
            _render_markdown(records, fit, extrap, device_name, total_mem_gb, args.compile),
            encoding="utf-8")

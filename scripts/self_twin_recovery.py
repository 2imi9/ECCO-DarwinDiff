...
optimizer holds all six parameters at the truth with loss ~1e-10 — against *box-generated*
targets the box map is **locally identifiable at Carroll and the method recovers it exactly**.
That is the clean method proof. Seeded from neutral mid-bounds, naive single-start Adam can
instead stall *orders of magnitude above* ~1e-10 with parameters ~20 % off; because the truth
is a sharp ~1e-10 minimum, that stall is an **optimization / initialization artifact (local
min or slow convergence), NOT intrinsic degeneracy** — genuine sloppiness would drift the
parameters even from start-at-truth.

SCOPE (important): this isolates the *method* + *optimization* only. It does **not** imply the
real multi-AOI fits are optimization-limited. Against REAL data the binding limits are
**observability**, not multi-start: the growth pair {Smallgrow, Biggrow} is unobservable by
construction (no real growth-rate data), ``diatomgraz`` needs a different/denser observable
(dense Darwin POSi/TRAC16, not yet staged), and the surrogate gap is **dimensional** (the 0-D
box homogenizes spatial structure, tracer CV → ~1e-15). None of those is fixable by a multi-start
or lr schedule. The honest target is the **4 observable params**. See ``STATUS.md`` and
``docs/research_notes/2026-06-27_box_homogenization_DEFINITIVE.md``.

...
**Headline experiment: DINN baseline** (1×1 conv, SST input, ~400 params). Can't saturate trivially → recovered Carroll-6 means carry identifiability information. Direct A/B vs nb20 DINN baseline (v2.0 7-tracer carbonate).

**Secondary: DINNDeep** (4-channel input, ~9.4K params) for continuity with nb15-20.

**Success criteria:**
- All 6 Carroll-6 params land within iron-pair quality range vs nb20 → Phase 2 hypothesis confirmed; 5-PFT box is the structural fix.
- Subset move closer, subset stay or degrade → per-PFT growth-rate-only isn't enough; v2.2.1 adds per-PFT half-saturations.
...
'''))
    ...
    cells.append(nbf.v4.new_markdown_cell(r'''## 4. Per-target Pearson r + recovered Carroll-6 vs Carroll-published

**Headline question:** do all 6 Carroll-6 parameters move into iron-pair-quality range (≤ ~40% off Carroll) when each governs a single specific PFT?

Compared to the nb20 (v2.0) DINN baseline — the same architecture and same forcing, only the box-model structure differs.
'''))
...
# === HEADLINE: 5-PFT DINN baseline (nb23) vs 2-PFT DINN baseline (nb20) ===
# nb20 DINN baseline reference from main, executed seed=0 (Track 1 v2.0
# headline). Carroll-6 published values are the calibration target.
NB20_DINN_MEANS = {
    "alpfe":      9.3833e-01,
    "scav_rat":   8.4458e-07,
    "Smallgrow":  2.4250e-01,  # 63% off Carroll (drifted)
    "Biggrow":    1.0916e+00,  # 153% off Carroll (drifted)
    "diatomgraz": 1.2724e-01,  # 85% off Carroll (drifted)
    "R_PICPOC":   1.9525e-01,  # 360% off Carroll (drifted)
}
print("\n=== HEADLINE: 5-PFT DINN baseline (nb23) vs 2-PFT DINN baseline (nb20) ===")
print(f"  {'param':<11s} {'nb20 |Δ|/Carroll':>20s} {'nb23 |Δ|/Carroll':>20s} {'closer?':>10s}")
b_closer = 0
for i, name in enumerate(PARAM_NAMES):
    p_b = r_baseline["params_final"][i].numpy()[ocean_mask].mean()
    p_nb20 = NB20_DINN_MEANS[name]
    pub = float(CARROLL_VALUES[i])
    rel_20 = abs(p_nb20 - pub) / abs(pub)
    rel_23 = abs(p_b - pub) / abs(pub)
    closer = "YES" if rel_23 < rel_20 else "no"
    if rel_23 < rel_20:
        b_closer += 1
    print(f"  {name:<11s} {rel_20:>20.3f} {rel_23:>20.3f} {closer:>10s}")
print(f"  -> {b_closer} of 6 parameters moved CLOSER vs nb20 (Phase 2 expected: all 6)")
'''))
...
'''))
    ...
    cells.append(nbf.v4.new_markdown_cell(r'''## 6. Interpretation — does the 5-PFT box "catch" Green's-functions?

The Phase 2 hypothesis was: by mapping each Carroll-6 parameter to a single specific PFT (instead of an average across multiple species), the 4-of-6 parameter drift collapses, matching the iron-pair quality (~1% `alpfe`, ~40% `scav_rat` off Carroll) v2.0 already achieved.

Read the headline table in §4 — if all 6 parameters move closer to Carroll's published optima vs nb20, the 5-PFT box structurally addresses the box-model bottleneck. If a subset stay drifted, that subset likely needs per-PFT half-saturations or per-PFT mortalities (deferred to v2.2.1).

Comparison reference values (v2.0 nb20 DINN baseline):
- `alpfe`: 0.938 (~1% off Carroll 0.928)  ← iron pair, calibration-grade
- `scav_rat`: 8.45e-7 (~40% off Carroll 6.03e-7)  ← iron pair, calibration-grade
- `Smallgrow`: 0.242 (~63% off Carroll 0.661)  ← drifted
- `Biggrow`: 1.092 (~153% off Carroll 0.431)  ← drifted
- `diatomgraz`: 0.127 (~85% off Carroll 0.830)  ← drifted
- `R_PICPOC`: 0.195 (~360% off Carroll 0.042)  ← drifted (biggest)

Phase 2 success criterion: nb23 DINN baseline pulls all 4 drifted parameters into ≤ ~40% off Carroll range, matching the iron-pair quality.
'''))
